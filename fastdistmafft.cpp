...
unsigned uAminoGroupCount = sizeof(AminoGroup)/sizeof(AminoGroup[0]);
...
static unsigned GetTuple(const unsigned uLetters[], unsigned n)
	{
	assert(uLetters[n] < uAminoGroupCount);
	assert(uLetters[n+1] < uAminoGroupCount);
	assert(uLetters[n+2] < uAminoGroupCount);
	assert(uLetters[n+3] < uAminoGroupCount);
	assert(uLetters[n+4] < uAminoGroupCount);
	assert(uLetters[n+5] < uAminoGroupCount);

	unsigned u1 = AminoGroup[uLetters[n]];
	unsigned u2 = AminoGroup[uLetters[n+1]];
	unsigned u3 = AminoGroup[uLetters[n+2]];
	unsigned u4 = AminoGroup[uLetters[n+3]];
	unsigned u5 = AminoGroup[uLetters[n+4]];
	unsigned u6 = AminoGroup[uLetters[n+5]];
...
// Convert to letters
	...
	for (unsigned uSeqIndex = 0; uSeqIndex < uSeqCount; ++uSeqIndex)
		...
		for (unsigned n = 0; n < uSeqLength; ++n)
			{
			char c = s[n];
			L[n] = CharToLetterAminoEx[c];
			assert(L[n] < uAminoGroupCount);
			}
		}
