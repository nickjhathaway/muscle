...
static unsigned GetTuple(const ProfPos *PP, unsigned uPos)
	{
	const unsigned t0 = PP[uPos].m_uAminoGroup;
	if (AAGROUP_MULTIPLE == t0)
		return EMPTY;

	const unsigned t1 = PP[uPos+1].m_uAminoGroup;
	if (AAGROUP_MULTIPLE == t1)
		return EMPTY;

	const unsigned t2 = PP[uPos+2].m_uAminoGroup;
	if (AAGROUP_MULTIPLE == t2)
		return EMPTY;

	const unsigned t3 = PP[uPos+3].m_uAminoGroup;
	if (AAGROUP_MULTIPLE == t3)
		return EMPTY;

	const unsigned t4 = PP[uPos+4].m_uAminoGroup;
	if (AAGROUP_MULTIPLE == t4)
		return EMPTY;
	...
	// Try to extend the match forwards
		...
		for (;;)
			...
			if (uLengthA - 1 == uEndPosA || uLengthB - 1 == uEndPosB)
				break;
			const unsigned uAAGroupA = PA[uEndPosA+1].m_uAminoGroup;
			if (AAGROUP_MULTIPLE == uAAGroupA)
				break;
			const unsigned uAAGroupB = PB[uEndPosB+1].m_uAminoGroup;
			if (AAGROUP_MULTIPLE == uAAGroupB)
				break;
